...
{
    // subscribe命令本身造成线程组的等待通道里面发生消息，这里只做订阅通道，不接受通道消息
    // 通道消息的接受专门在observer_channel_message函数中的独立线程中进行
    // 只负责发送命令，不阻塞接受redis server的相应消息，否则和notifyMsg线程抢占相应资源
    ...
    // redisBufferWrite额可以循环发送缓冲区，直到缓冲区的数据发送完毕(done被置1)
    int done = 0;
    while (!done) 
    {
...
// 初始化向业务层上报通道中的回调对象
void Redis::init_notify_channel_message(function<void(int, string)> fn)
{
